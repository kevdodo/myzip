...
fn find_match_rle(buffer: &Vec<u8>, buffer_idx: &usize, 
    ...
    temp_matches: FxHashMap<(u8, u8, u8), usize>, max_len: &mut usize, curr_dist: &mut usize){
    ...
    if let Some(index) = temp_matches.get(&next_3_bytes){
        ...
        true_matches.dict[(*buffer_idx + 1) & LZ_DICT_SIZE_MASK] = buffer[*buffer_idx + 1];
    ...
    'index_loop: while index != 0{
        ...
        'found: loop {
            ...
            for _ in 0..3{
                ...
                let element1 = true_matches.dict[index1];
                let element2 = true_matches.dict[index2];
                let element3 = true_matches.dict[index3];
            
                if element1 == buffer[*buffer_idx] && element2 == buffer[*buffer_idx + 1] && element3 == buffer[*buffer_idx + 2] {
                    break 'found;
                }
                index = true_matches.next[index as usize];
            }
        }
    ...
    while buffer_idx < buffer.len(){
        ...
        match matches{
            Some(current_match_val) =>{
                ...
                buffer_idx += length;
                // curr_hash.add_char(buffer[length-3]);
                // curr_hash.add_char(buffer[length-2]);
                // curr_hash.add_char(buffer[length-1]);   
